...
contract MagicStargateLpHarvestor is Operatable, FeeCollectable {
    ...
    function claimable() public view returns (uint256) {
        ICurveRewardGauge staking = IMagicStargateLpRewardHandler(address(vault)).staking();
    ...
    function run(uint256 minLp, IERC20 tokenIn, uint256 maxAmountIn, bytes memory swapData) external onlyOperators {
        IMagicStargateLpRewardHandler(address(vault)).harvest(address(this));
    ...
    function _compoundFromToken(
        ...
        IMagicStargateLpRewardHandler(address(vault)).distributeRewards(assetAmount);
