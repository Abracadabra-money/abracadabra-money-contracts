...
contract Rewarder is IRewarder {
     ...
     */
    function deposit(uint256 _amount, address from) external onlyCauldron {
    ...
    function _repayLoan(address to, uint256 amount, uint256 part) internal {
        mim.safeTransfer(address(degenBox), amount + 1);
        degenBox.deposit(mim, address(degenBox), address(degenBox), amount + 1, 0);
    ...
    function _harvest(UserInfo memory user, address to) internal returns (UserInfo memory) {
        ...
        uint256 pendingRewards = uint256(accumulatedMim - user.rewardDebt) - 1;
        ...
        if (pendingRewards != 0) {
            ...
            if (elastic >= pendingRewards) {
                ...
                _repayLoan(to, elastic, borrowPart);
     ...
     */
    function withdraw(address from, uint256 _amount) external onlyCauldron {
        ...
        userInfo[from] = _harvest(user, from);
