...
contract MagicInternetMoneyMock is ERC20, BoringOwnable {
...
contract CauldronV2Mock is BoringOwnable, IMasterContract {
    using BoringMath for uint256;
    ...
    CauldronV2Mock public immutable masterContract;
    ...
    constructor(IBentoBoxV1 bentoBox_, IERC20 magicInternetMoney_, address _masterContract) {
        bentoBox = bentoBox_;
        magicInternetMoney = magicInternetMoney_;
        masterContract = CauldronV2Mock(_masterContract);
    }
    ...
    function _isSolvent(address user, uint256 _exchangeRate) internal view returns (bool) {
        ...
        uint256 left = bentoBox.toAmount(
            collateral,
            collateralShare.mul(EXCHANGE_RATE_PRECISION / COLLATERIZATION_RATE_PRECISION).mul(COLLATERIZATION_RATE),
            false
        );
        uint256 right = borrowPart.mul(_totalBorrow.elastic).mul(_exchangeRate) / _totalBorrow.base;
    ...
    function liquidate(address[] calldata users, uint256[] calldata maxBorrowParts, address to, ISwapperV1) public {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            console2.log("exchangeRate", _exchangeRate);
            console2.log("userCollateralShare", userCollateralShare[user]);
            console2.log("userBorrowPart", userBorrowPart[user]);
            console2.log("userBorrowPart to elastic", _totalBorrow.toElastic(userBorrowPart[user], true));
            console2.log("userCollateral  amount", bentoBox.toAmount(collateral, userCollateralShare[user], true));
            ...
            if (!_isSolvent(user, _exchangeRate)) {
                ...
                uint256 collateralShare = bentoBoxTotals.toBase(
                    _totalBorrow.toElastic(borrowPart, false).mul(LIQUIDATION_MULTIPLIER).mul(_exchangeRate) /
                        (LIQUIDATION_MULTIPLIER_PRECISION * EXCHANGE_RATE_PRECISION),
                    false
                );
                console2.log(" collateralShare / userCollateralShare", collateralShare, userCollateralShare[user]);

                if (collateralShare > userCollateralShare[user]) {
                    console2.log("collateralShare > userCollateralShare[user]");
                }
    ...
    function reduceSupply(uint256 amount) public {
        ...
        MagicInternetMoneyMock(address(magicInternetMoney)).burn(amount);
    }
}
