...
import {MagicStargateLpScript} from "script/MagicStargateLp.s.sol";
...
import {MagicStargateLp} from "tokens/MagicStargateLp.sol";
import {MagicStargateLpHarvestor} from "periphery/MagicStargateLpHarvestor.sol";
import {MathLib} from "libraries/MathLib.sol";
import {MagicStargateLpRewardHandler, IMagicStargateLpRewardHandler, MagicStargateLpRewardHandlerDataV1} from "periphery/MagicStargateLpRewardHandler.sol";

contract MagicStargateLpRewardHandlerV2Mock is MagicStargateLpRewardHandlerDataV1 {
...
abstract contract MagicStargateLpTestBase is BaseTest {
    ...
    function afterInitialize() public {
        ...
        staking = IMagicStargateLpRewardHandler(address(vault)).staking();
    ...
    function testProtectedDepositAndWithdrawFunctions() public {
        ...
        IMagicStargateLpRewardHandler(address(vault)).stakeAsset(123);

        vm.expectRevert(abi.encodeWithSignature("ErrPrivateFunction()"));
        IMagicStargateLpRewardHandler(address(vault)).unstakeAsset(123);

        // call throught fallback function from EOA
        {
            bytes memory data = abi.encodeWithSelector(MagicStargateLpRewardHandler.stakeAsset.selector, 123);
            vm.expectRevert(abi.encodeWithSignature("ErrPrivateFunction()"));
        ...
        {
            bytes memory data = abi.encodeWithSelector(MagicStargateLpRewardHandler.unstakeAsset.selector, 123);
            vm.expectRevert(abi.encodeWithSignature("ErrPrivateFunction()"));
            (bool success, ) = address(vault).call{value: 0}(data);
            assertTrue(success);
        }
    ...
    function testUpgradeRewardHandler() internal {
        MagicStargateLpRewardHandlerV2Mock newHandler = new MagicStargateLpRewardHandlerV2Mock();
        IMagicStargateLpRewardHandler previousHandler = IMagicStargateLpRewardHandler(vault.rewardHandler());

        vm.startPrank(vault.owner());
        MagicStargateLpRewardHandler(address(vault)).harvest(address(this));

        // check random slot storage value for handler and wrapper
        ICurveRewardGauge previousValue1 = IMagicStargateLpRewardHandler(address(vault)).staking();
        string memory previousValue2 = vault.name();

        // upgrade the handler
        vm.expectEmit(true, true, true, true);
        emit LogRewardHandlerChanged(previousHandler, IMagicStargateLpRewardHandler(address(newHandler)));
        vault.setRewardHandler(IMagicStargateLpRewardHandler(address(newHandler)));

        ICurveRewardGauge _staking = IMagicStargateLpRewardHandler(address(vault)).staking();
        assertEq(address(_staking), address(previousValue1));
        assertEq(vault.name(), previousValue2);

        MagicStargateLpRewardHandlerV2Mock(address(vault)).handleFunctionWithANewName(111, ICurveRewardGauge(address(0)), "abracadabra");

        _staking = IMagicStargateLpRewardHandler(address(vault)).staking();

        assertEq(address(_staking), address(0));
        assertEq(vault.name(), "abracadabra");
        assertEq(MagicStargateLpRewardHandlerV2Mock(address(vault)).newSlot(), 111);
        vm.stopPrank();
    }
    ...
    function testTotalAssetsMatchesBalanceOf(uint256 amount1, uint256 amount2, uint256 amount3, uint256 rewards) public {
        ...
        vm.expectRevert();
        IMagicStargateLpRewardHandler(address(vault)).distributeRewards(rewards);
        _assertStakingMatchTotalAssets();
        pushPrank(vault.owner());
    ...
    function testStakingSkimming(uint256 amount1, uint256 amount2, uint256 amount3, uint256 rewards) public {
        ...
        if (rewards > 1) {
            ...
            (uint256 fromStaking, uint256 fromVault) = IMagicStargateLpRewardHandler(address(vault)).skimAssets();
            ...
            (fromStaking, fromVault) = IMagicStargateLpRewardHandler(address(vault)).skimAssets();
...
contract MagicStargateLpKavaMimUsdtVaultTest is MagicStargateLpTestBase {
    using SafeApprove for IERC20;
    ...
    MagicStargateLpScript script;
    IERC20 usdt;

    function setUp() public override {
        fork(ChainId.Kava, 5816652);

        script = new MagicStargateLpScript();
