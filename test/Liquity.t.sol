...
contract LiquityTest is BaseTest {
    ...
    address constant lusdWhale = 0x99C9fc46f92E8a1c0deC1b1747d010903E884bE1;
    IBentoBoxV1 degenBox;
    LiquityStabilityPoolStrategy strategyV1 = LiquityStabilityPoolStrategy(payable(0x1EdC13C5FC1C6e0731AE4fC1Bc4Cd6570bBc755C));
    LiquityStabilityPoolStrategy strategy;
    ILiquityStabilityPool pool;
    ERC20 lqtyToken;
    ERC20 lusdToken;

    function setUp() public override {
        forkMainnet(15875206);
        super.setUp();
        ...
        pool = ILiquityStabilityPool(constants.getAddress("mainnet.liquity.stabilityPool"));
    ...
    function _activateStrategy() private {
        uint256 amountLpDegenBox = lusdToken.balanceOf(address(degenBox));
        uint amountStaked = pool.getCompoundedLUSDDeposit(address(strategyV1));
        assertGt(amountStaked, 0);
      ...
      /*  // check that v1 exit correctly
        ...
        assertGt(lusdToken.balanceOf(address(degenBox)), amountLpDegenBox);
        assertEq(lusdToken.balanceOf(wftmmimStrategyV1), 0);
